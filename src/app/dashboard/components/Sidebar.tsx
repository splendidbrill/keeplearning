...
export const Sidebar: React.FC<SidebarProps> = ({ user }) => {
  ...
  const menuRef = useRef<HTMLDivElement>(null);
  const router = useRouter();
  const pathname = usePathname(); // Get current path to highlight active button
  const supabase = createClient();
  ...
  const handleNavigation = (path: string) => {
    router.push(path);
    // Optional: Close sidebar on mobile if you implement mobile responsiveness later
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 py-6 px-4 space-y-2">
        ...
        {/* 2. Profile Button (Visible in main nav too) */}
        <button 
