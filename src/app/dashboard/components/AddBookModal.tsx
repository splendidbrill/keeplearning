...
import { ingestBook } from '../../../lib/n8n'; // Ensure this path is correct
...
interface AddBookModalProps {
  ...
  onClose: () => void;
  subjectName: string;
  subjectId: string; // <--- ADDED: Needed for DB insert
  user: User;        // <--- ADDED: Needed for DB insert
...
export const AddBookModal: React.FC<AddBookModalProps> = ({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Upload PDF to Supabase Storage (if new file selected)
      if (file) {
        const fileExt = file.name.split('.').pop();
        const fileName = `${user.id}/${Date.now()}_${file.name.replace(/[^a-zA-Z0-9]/g, '_')}.${fileExt}`;
        
        const { error: uploadError } = await supabase.storage
          .from('book-files') // Make sure this bucket exists in Supabase Storage!
      ...
      const newBookData = {
        ...
        description,
        file_url: fileUrl,
        // If editing, we might want to keep old fileUrl if no new file
      ...
      // 3. Trigger n8n AI Ingestion (Background Process)
      ...
      onAdd({
        ...
        color: '#fbbf24', // Default color or calculate generic
        file: file,
        // @ts-ignore
